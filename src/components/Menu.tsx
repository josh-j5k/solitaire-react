...
export default function Menu() {
	...
	function changeDifficulty(e: string) {
		...
		if (difficulty !== e) {
			dispatch(updateDifficultyState(e))
			startGame()
		}
	}
	async function fetchLeaderboard() {
		dispatch(updateLeaderboardState(true))
		dispatch(updateMenuToggledState(false))
	...
	function help() {
		...
		dispatch(updateHowToPlayState(true))
		dispatch(updateMenuToggledState(false))
	}
	return (
		<div
			className={`w-1/4 h-screen fixed top-0 left-0 py-24 bg-black/75 z-50 transition-transform duration-150 ${
				menuToggled ? "translate-x-0" : "-translate-x-full"
			}`}
		>
			<nav className="flex flex-col items-center gap-8">
				<button aria-label="closeMenu" className="absolute top-6 right-4">
					<span>
						<svg
							viewBox="0 0 24 24"
							className="w-6 h-8"
				...
				<button onClick={startGame} className="flex gap-4 items-center group">
					<span>
						<svg
							viewBox="0 0 32 32"
							version="1.1"
							...
							className="w-10 h-10 -lg:w-6 -lg:h-6 group-hover:scale-90 transition-transform"
							fill="#000000"
						>
							<g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
							<g
					...
					<span className="capitalize text-3xl -lg:text-2xl font-bold text-white group-hover:text-[#00a3f5]">
						new game
					</span>
				</button>
				<button
					onClick={pauseAndPlayGame}
					...
					<span>
						{pause ? (
							<svg
								viewBox="0 0 24 24"
								fill="none"
								className="w-10 h-10 -lg:h-6 -lg:w-6 group-hover:scale-90 transition-transform"
								xmlns="http://www.w3.org/2000/svg"
							>
								<g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
								<g
				...
				<div className="relative">
					...
					{dropdown && (
						<div className="bg-black/70 absolute translate-x-[110%] top-0 w-11/12 p-8 z-30 shadow">
							<ul className="flex flex-col items-center gap-3 text-3xl -lg:text-2xl">
								<button
									onClick={() => changeDifficulty("easy")}
									className={`hover:text-blue-500 capitalize ${
										difficulty === "easy" && "text-blue-500"
									}`}
								>
									easy
								</button>
								<button
									...
									className={`hover:text-blue-500 capitalize ${
										difficulty == "medium" && "text-blue-500"
									}`}
								>
									medium
								</button>
				...
				<button onClick={help} className=" flex gap-4 items-center group">
					<span>
						<svg
							className="w-10 h-10 -lg:h-6 -lg:w-6 group-hover:scale-90 transition-transform"
							version="1.1"
